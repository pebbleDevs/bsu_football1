...
// MACRO SECTION
#define MY_UUID { 0x19, 0xE4, 0x75, 0x31, 0x91, 0x47, 0x48, 0x9F, 0x81, 0x73, 0xD1, 0x9A, 0x56, 0x4D, 0x51, 0xFD }
PBL_APP_INFO(MY_UUID,
             "BSU FOOTBALL 1", "mApps Lab",
             1, 0, /* App version */
...
void creditBanner()
{
 ...
 //initilizing the creditline layer
  ...
  layer_set_frame(&text_creditLine1_layer.layer, GRect(0,100,144,30)); //GRect(X,Y, Width, Height)
  //Font attribute
  ...
  layer_set_frame(&text_creditLine2_layer.layer, GRect(0,120,144,30));
  //Font attribute
  ...
  layer_set_frame(&text_versionInfo_layer.layer, GRect(55,145,60,20));
  //Font attribute
...
void handle_init(AppContextRef ctx) {
  ...
  bmp_init_container(RESOURCE_ID_BSU_BOW, &background_image);
  layer_set_frame(&background_image.layer.layer, GRect(0,-5,144,115));
  ...
  layer_set_frame(&text_time_layer.layer, GRect(0,105,144,80));
  //Font atterbutes
  custom_font1 = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_VARSITY_42));
  //gunplay_40 = fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD);
  text_layer_set_font(&text_time_layer, custom_font1);
  layer_add_child(&window.layer, &text_time_layer.layer);
  
  //initilizing the AM_PM layer  
  text_layer_init(&text_ampm_layer, GRect(116,100,25,20));
